...
{
    return std::max({0
#define FIELD(_, __, ___, n, ____) , n
...
{
    std::vector<unsigned char> answer(order_execution_bitfield_num(), 0);
#define FIELD(_, __, ___, bitfield, bit) \
    answer[bitfield - 1] |= bit;
#include "order_execution_opt_fields.inl"
    return answer;
}
...
{
    std::vector<unsigned char> answer(order_restatement_bitfield_num(), 0);
#define FIELD(_, __, ___, bitfield, bit) \
    answer[bitfield - 1] |= bit;
#include "order_restatement_opt_fields.inl"
    return answer;
